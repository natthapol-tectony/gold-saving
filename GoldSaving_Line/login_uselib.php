...
define('LINE_LOGIN_CHANNEL_ID','1655133254');
define('LINE_LOGIN_CHANNEL_SECRET','5f512368c30223f39adabb11734842d3');
define('LINE_LOGIN_CALLBACK_URL','https://www.devserv.me/GoldSaving/login_uselib_callback.php');
...
if(isset($_SESSION['ses_login_userData_val']) && $_SESSION['ses_login_userData_val']!=""){
  ...
  //  print_r($lineUserData); 
	...
	$_SESSION['UserID']=$lineUserData['sub'];
	$_SESSION['DisplayName']=$lineUserData['name'];
	$_SESSION['Picture']=$lineUserData['picture'];
	//echo "<script>window.open('http://54.83.240.139/?role=User&uid=$lineUserData[sub]','_top');</script>";
	?>
	<form name="yourform" id="yourform" method="POST" action="http://54.83.240.139/">
    <input type="hidden" name="uid" value="<?=$lineUserData['sub']?>">
